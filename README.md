...
| ショップ（授与品） | `#授与所` の「授与品を見る」: 色守り（名前の色・30 日）・称号・花吹雪（#境内 にお祝い）・贈り物（花びらを贈る・氏子以上）・絵馬の奉納（自己紹介を 7 日ピン留め）・おみくじもう 1 回・免罪符。品物・値段・説明・日数・販売のオン／オフは管理画面の「ショップ」（宮司）で変えられる。Discord の操作に失敗したら花びらを戻す |
| 初期配布 | 入鯖が承認されたときに花びらを 1 回だけ（既定 3000）。今いる人には「設定」から 1 回だけ配れる |
...
**自動（おすすめ）**: BOT のロールに「管理者」を付けてから `docker compose run --rm --build setup --guild （サーバー ID）` を実行すると、下の 1〜3 と「3. ID を設定ファイルに書く」をまとめて行う（テスト用は `--minimal`、確認だけは `--dry-run`、使わないチャンネルの削除と並べ替えもするなら `--tidy`）。終わったら「管理者」を OFF に戻す。
