...
| 鯖内通貨 | 🌸花びら（仮の名前）。通話と朱印で貯まり、免罪符に使う（[admin.md](admin.md) §3.5.1） |
| 厄（警告） | 1 つ目で注意、2 つ目で BAN、重大な違反は一発 BAN。免罪符で 1 回だけ祓える |
...
| ③ 通話から | 通話に入ると、通話のチャットに「この通話の人に朱印を押す」が出る（一緒にいる人がボタンで並ぶ） | 通話したあとに押すとき |
...
|---|---|
| `#御触書` | お知らせ |
| `#絵馬-男性`・`#絵馬-女性` | 自己紹介（🪧 絵馬殿。いちばん下にひな形） |
| `#慶事` | 昇格・称号の発表 |
| `#番付` | ランキング（今月・累計の上位） |
...
| `#おみくじ` | BOT コマンド |
| `🔊 拝殿` | みんなの雑談 VC（固定） |
| `➕ 縁側をひらく` | 入ると自分の雑談 VC（🍵 〇〇の縁側）が作られる（§3.3） |
| `🔊 奥の院` | 放置（AFK） |
...
| `➕ 屋台をひらく` | 入るとゲーム用の VC（🎮 〇〇の屋台）が作られる。ゲーム名は部屋主が名前を変えて付ける（§3.3） |
| `🔊 神楽殿` | 歌・カラオケ・配信（固定） |
...
|---|---|
| `#宿帳` | 寝落ち通話の募集・おやすみの挨拶 |
| `➕ 宿坊をひらく` | 入ると寝落ち VC（🌙 〇〇の宿坊）が作られる（全年齢・**鍵をかけられない**） |
...
**実装済み（2026-09）。** 固定の VC は拝殿・神楽殿・奥の院・寄合だけにし、ほかは入口から作る。

| 入口 | できる部屋 | 部屋主ができること |
|---|---|---|
| `➕ 縁側をひらく` | 🍵 〇〇の縁側（雑談） | 名前変更・人数制限・人を抜けさせる |
| `➕ 屋台をひらく` | 🎮 〇〇の屋台（ゲーム。ゲーム名は名前変更で） | 同上 |
| `➕ 宿坊をひらく` | 🌙 〇〇の宿坊（寝落ち） | 同上 |
| `➕ 宵宮の部屋をひらく` | 🍶 〇〇の部屋（18 歳以上・年齢制限） | 同上 |

- 部屋はカテゴリと同じ見える範囲で作る（宵宮の部屋は宵参りの人だけ）。部屋主には、その部屋だけの「チャンネルの管理」「メンバーを移動」を付ける（Discord の「チャンネルを編集」から操作。見える範囲は変えられない）。
- **全年齢エリアの部屋は非公開にできない**（誰がいるか全員から見える）。
- 自分の部屋があるのに入口に入ると、2 つ目は作らずその部屋へ戻す。BOT が止まっていた間に空になった部屋は、起動時と 1 分ごとに消す。
- 入口の名前とできる部屋の名前は `config/guild.json` の `tempVoice.hubs`（セットアップが書く）。
- 鍵（招待制）の部屋と、部屋の中のボタンでの操作は未実装。
