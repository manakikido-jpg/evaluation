...
| 🔞 成人 | 18 歳以上と申告した人が申請 → 運営承認 | 成人エリア（§2.5）の閲覧・参加。役職とは別に付く |
...
**方針（確定）: 18 歳未満も参加できる。ただし 18 歳以上だけが入れる「成人エリア」を別に設ける。**

#### 基本ルール
- 参加できるのは **13 歳以上**（Discord の利用規約）。
- 成人エリア以外は**全年齢の場所**。18 歳未満がいる前提で、成人向けの話題は成人エリアの中だけにする。
- 成人エリアの話題は「お酒・夜更かしトーク・大人の雑談」などを想定。**性的な内容の通話・投稿を目的とした場所にはしない**（未成年が同じ鯖にいるため、トラブルになったときの危険が大きい）。

#### 年齢の確認
- 入鯖申請フォームで **年齢区分** を必須にする: `13〜17 歳` / `18 歳以上`
  - 生年月日は聞かない・保存しない（区分だけ記録）。
- 成人エリアに入るには **2 重のカギ** をかける:
  1. **🔞 成人ロール**: 申請時に 18 歳以上と答えた人だけが `#しきたり` のボタンから申請でき、運営が承認して付与
  2. **Discord の「年齢制限チャンネル」設定**: 成人エリアのテキスト・ボイスチャンネル全部に設定する。入るときに Discord 側でも年齢の確認が入る
- 18 歳未満と答えた人は、成人ロールを申請できない（ボタンを押しても BOT が断る）。
- 18 歳になったら、運営に言えば区分を更新できる。
- 成人エリアに 18 歳未満がいると分かったら、BOT コマンドで即座に成人ロールを外し、区分を 13〜17 歳に直す。

#### 18 歳未満を守るためのルール（`#しきたり` に明記）
- **大人から 18 歳未満への、恋愛・性的な目的での DM や誘いは禁止**。発覚したら即 BAN し、必要に応じて Discord に通報する。
- 鯖の外（他の SNS・オフ会など）で 18 歳未満と会う約束をすることを禁止。
- 18 歳未満かどうかは**ロールとして公開しない**（「未成年」が目印になって狙われるのを防ぐ）。代わりに「成人エリア以外は全年齢」を全員のルールにする。
- 困ったとき用に、運営へ匿名で相談できる窓口（BOT の `/soudan` コマンド → `#寄合` に匿名で届く）を用意する。

#### 評価（朱印）との関係
- 朱印・ご縁・役職は年齢に関係なく同じ（成人エリアでの活動も普段と同じく数える）。
- 成人ロールは役職（参拝者〜総代）とは**別のロール**で、ご縁には影響しない。
...
erDiagram
    ...
    MEMBER {
      ...
      enum age_group "minor(13-17)|adult(18+) 生年月日は持たない"
      bool adult_access "成人エリアの許可"
      int rank_id FK
...
| 18 歳未満が成人エリアに入る | 申告 + 運営承認 + Discord の年齢制限チャンネルの 2 重のカギ / 発覚したら即ロール解除 |
| 大人による 18 歳未満への接触 | 恋愛・性的目的の DM や鯖外での約束を禁止、即 BAN / 匿名相談 `/soudan` |
...
- 何人にでも押せるが、同じ人には 1 回だけ
- コンセプトは「御朱印」、役職名も確定（[concept.md](concept.md) §3）
- サーバー名は「咲楽ノ宮（さくらのみや）」
- 18 歳未満も参加可。18 歳以上だけの成人エリアを別に設ける（§2.5）

## 9. まだ教えてほしいこと

1. **だいたいのメンバー数**（昇格に必要なご縁の数を合わせるため）
